...
export async function action({ request }: Route.ActionArgs) {
  ...
  const actionType = String(data._action ?? data.intent ?? "")
    .trim()
    .toLowerCase();
...
**Routes already using `parseActionRequest`**: api.contact-audience, api.contact-audience.bulk-delete, api.audiences, api.campaigns, workspaces*.$id.campaigns.$selected_id.settings, workspaces*.$id.campaigns.$selected_id.queue.
