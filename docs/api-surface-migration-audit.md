...
| `/api/workspace-api-keys` | `workspaceAdmin` | ~~The floor is `requireMemberManager`, which denies only `caller` — a **member** floor, not admin. Worth noting separately: a plain member can mint a key with any capability scope.~~ **RESOLVED (issue #1264).** The declaration was right and the code was wrong: `requireApiKeyManager` now enforces the admin floor on list/create/delete, and `assertScopesWithinActorRole` caps a new key's scopes at the creator's own role capabilities. Both flat and scoped routes funnel through the same service functions, so one fix covers both. |
