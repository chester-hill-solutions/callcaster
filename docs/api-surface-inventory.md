...
| `/api/numbers` | GET, POST | User API | sessionOnly | yes | `routes/api+/numbers.tsx` | `docs/api-workspace-admin.md` | Flat alias for /api/workspaces/:workspaceId/numbers used by the number purchase wizard. POST requires the member role or above; the caller role receives 403. |
...
| `/api/workspaces/:workspaceId/numbers` | GET, POST | User API | sessionOnly | yes | `routes/api+/workspaces+/$workspaceId/numbers.route.tsx` | `docs/api-telephony-provisioning.md` | GET lists numbers for any workspace member. POST (purchase) requires the member role or above; the caller role receives 403. |
