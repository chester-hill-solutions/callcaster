-- Replace auth.uid() with the v2 identity in select_and_update_campaign_contacts
-- (#1885, chunk 2).
--
-- auth.uid() is our Supabase-era shim (drizzle/0001_auth_uid_shim.sql); the v2
-- equivalent is app.current_user_id, set by withAppCurrentUser for every caller
-- of this RPC (app/lib/db-rpc.server.ts). Inlining it removes the function's
-- dependency on the auth schema ahead of DROP SCHEMA auth.
--
-- Body copied verbatim from 20260807130000_manual_dial_claim_attempt_count.sql
-- except the actor expression. That migration's concurrency fix (FOR UPDATE
-- SKIP LOCKED, household bundling, attempt_count/claimed_at writes) is preserved.
...
BEGIN
    ...
    SET queue_state = 'assigned',
        ...
        -- and abandoned claims are stranded (see 20260807130000 header).
