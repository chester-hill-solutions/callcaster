...
-- Source: @chester-hill-solutions/auth-postgres@0.3.0
--   WORKSPACE_FEATURE_AUTHZ_DDL, WORKSPACE_INVITATION_DDL, testing DDL workspace_member fragment
-- CallCaster adaptations:
--   1) Rename enum workspace_role → workspace_users_role (name collides with CHS table).
--   2) Omit FKs to workspace(id): workspace.id is uuid; package DDL uses text FKs.
--   3) workspace_member.user_id remains text (published package shape).
--   4) Seed PRODUCT_ROLE_IDS (owner/admin/member/caller), not testing SYSTEM_ROLES_SEED_DDL.
-- SEC-07 continues using workspace_users + in-app capability matrix until a later PR.
...
--    (WORKSPACE_FEATURE_AUTHZ_DDL; workspace FKs omitted — uuid vs text)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. workspace_member (testing DDL equivalent; role_id from day one)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. workspace_invitation (WORKSPACE_INVITATION_DDL; workspace FK omitted)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Global product role seeds (PRODUCT_ROLE_IDS / PRODUCT_ROLE_RANKS)
-- ---------------------------------------------------------------------------
