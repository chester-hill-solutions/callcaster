...
export const coverageDir = path.join(repoRoot, "coverage");

export const lcovInputs = [
  {
    name: "vitest-node",
...
/** @param {string} sf */
export function normalizeSf(sf) {
...
/** @returns {Map<string, FileCov>} */
export function mergeCovMaps(maps) {
  /** @type {Map<string, FileCov>} */
...
export function serializeLcov(map) {
...
export function isTrivialSourceFile(filePath) {
  try {
...
/**
 * @param {Map<string, FileCov>} covMap
 * @param {Set<string>} requiredFiles
 * @returns {{ok: boolean, failures: Array<{file: string, kind: string, have: number, total: number}>}}
 */
export function checkAll100(covMap, requiredFiles) {
  const failures = [];
  for (const file of requiredFiles) {
    const fc = covMap.get(file);
    if (!fc) {
      failures.push({ file, kind: "missing", have: 0, total: 1 });
      continue;
    }
...
/** @returns {Map<string, FileCov>} */
export function loadMergedCoverageMap() {
  for (const i of lcovInputs) {
  ...
  const parsed = lcovInputs.map((i) =>
    parseLcov(fs.readFileSync(i.path, "utf8")),
  );
  return mergeCovMaps(parsed);
}

/**
 * @param {Array<{file: string, kind: string, have: number, total: number}>} failures
 */
export function formatStrictFailures(failures) {
  /** @type {Map<string, {rel: string, metrics: Array<{kind: string, have: number, total: number}>}>} */
  const byFile = new Map();
  for (const f of failures) {
    const rel = path.relative(repoRoot, f.file);
    let row = byFile.get(rel);
    if (!row) {
      row = { rel, metrics: [] };
      byFile.set(rel, row);
    }
    row.metrics.push({ kind: f.kind, have: f.have, total: f.total });
  }

  const sorted = [...byFile.values()].sort((a, b) => {
  const area = (r) => r.rel.split(path.sep).slice(0, 2).join("/");
    const areaCmp = area(a).localeCompare(area(b));
  ...
  for (const row of sorted) {
    const parts = row.metrics.map(
      (m) => `${m.kind} ${m.have}/${m.total}`,
    );
    lines.push(`${row.rel}\t${parts.join(", ")}`);
  }
  return lines;
}
