...
export const KeyMessageMetrics = ({
  ...
  const getRate = (count: number): string => {
  ...
  const successCount =
    (totalsByDisposition?.delivered || 0) + (totalsByDisposition?.sent || 0);
  const failedCount =
    (totalsByDisposition?.failed || 0) + (totalsByDisposition?.undelivered || 0);
  const successRate = getRate(successCount);
  const failedRate = getRate(failedCount);
  ...
  return (
    <div className="mt-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-lg bg-blue-100 p-4">
          <p className="text-lg font-semibold text-blue-500">Success Rate</p>
          <p className="text-3xl font-bold text-blue-800">{successRate}%</p>
        </div>
