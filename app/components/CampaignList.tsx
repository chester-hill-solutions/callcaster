...
import CampaignStatusIndicator from "./CampaignHomeScreen/CampaignStatusIndicator";
...
const CampaignsList = ({ campaigns, userRole, setCampaignsListOpen }) => (
  <Card className="flex flex-auto flex-col border-none bg-secondary dark:bg-blue-950 ">
    ...
    <div className="flex flex-grow flex-col justify-between">
            ...
            !draftNotAllowed && (
              ...
              >
                <div className="flex w-full items-center justify-between">
                  <p>{row.title || `Unnamed campaign ${i + 1}`}</p>
                  {row.status && (
                    <CampaignStatusIndicator campaignStatus={row.status} />
                  )}
                </div>
              </NavLink>
            )
      ...
      <nav className="">
        <NavLink
          className={`flex items-center justify-center rounded-b-md bg-gray-100 px-4 py-2 font-Zilla-Slab text-sm font-medium transition-colors hover:bg-white dark:bg-zinc-900 dark:hover:bg-zinc-500`}
          to={"#"}
        >
          {" "}
