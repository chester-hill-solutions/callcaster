...
export const action = async ({ request }) => {
    ...
    const { campaign_id, title, status, type, dial_type, group_household_queue, caller_id, start_date, end_date, voicemail, questions, workspace } = await request.json();
    let tableKey = type === 'live_call' ? 'live_campaign' : null;

    try {
        const { data: campaign, error: campaignError } = await supabase
            .from('campaign')
            .update({ title, status, type, dial_type, group_household_queue, caller_id, start_date, end_date, workspace })
            .eq('id', campaign_id)
            .select();
        ...
        if (tableKey) {
            const { data: detailsData, error: campaignDetailsError } = await supabase
                .from(tableKey)
                .update({ voicemail, questions, workspace })
