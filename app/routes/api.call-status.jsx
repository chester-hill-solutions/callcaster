...
export const action = async ({ request }) => {
    ...
    if (parsedBody.CallStatus === 'failed') {
        const { data: attempt, error: attemptError } = await supabase.from('outreach_attempt').update({ ended_at: new Date(), disposition: 'failed' }).eq('id', data[0].outreach_attempt_id).select();
    }
