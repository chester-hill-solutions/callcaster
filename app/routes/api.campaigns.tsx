...
export const action = async ({ request }: { request: Request }) => {
    const { supabaseClient: supabase } = await createSupabaseServerClient(request);
    ...
    if (type === 'live_call') tableKey = 'live_campaign';
    ...
    try {
        ...
        if (campaignError) {
            if (campaignError.code === '23505') {
                return json({ error: 'A campaign with this title already exists in the workspace' }, { status: 400 });
            }
            throw campaignError;
        }
