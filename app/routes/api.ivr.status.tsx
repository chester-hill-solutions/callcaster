...
const updateResult = async (
  ...
  outreach_attempt_id: string | undefined,
...
function findVoicemailPage(
  pagesObject: Script["steps"]["pages"] | undefined,
): Script["steps"]["pages"][string] | null {
...
const handleVoicemail = async (
  ...
  callSid: string,
  dbCall: Call,
  campaign: Campaign & { ivr_campaign: [IVRCampaign & { script: Script }] },
  supabase: SupabaseClient,
): Promise<void> => {
  ...
  const step = findVoicemailPage(campaign.ivr_campaign[0].script?.steps?.pages);
  if (!step) {
    await call.update({
...
export const action = async ({ request }: { request: Request }) => {
  ...
  try {
    const { data: dbCall, error: callError } = await supabase
      .from("call")
      .select(
        "outreach_attempt_id, workspace, campaign(*, ivr_campaign(*, script(*)))",
      )
      .eq("sid", callSid)
      .single();
    ...
    if (callStatus === "failed" || callStatus === "no-answer") {
      ...
      parsedBody.AnsweredBy &&
      parsedBody.AnsweredBy.includes("machine") &&
      !parsedBody.AnsweredBy.includes("other") &&
      ...
      await handleVoicemail(
        ...
        callSid,
        dbCall as Call,
        dbCall.campaign as Campaign & {
          ivr_campaign: [IVRCampaign & { script: Script }];
        },
        supabase,
      );
    ...
    } else if (parsedBody.CallbackSource === "call-progress-events") {
      await handleCallStatusUpdate(
        supabase,
        callSid,
        callStatus,
