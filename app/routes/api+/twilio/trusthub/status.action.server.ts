...
export const action = defineAction({
  // NOTE: the Twilio signature check stays in `handler` (not `auth`) because
  // it must run AFTER the bundle SID is read from the form body and the
  // workspace resolved — validation uses that workspace's subaccount token.
