...
export default function Audience() {
  ...
  const initQuestions = useMemo(
    () => [...(pageData.length > 0 && pageData[0].campaignDetails?.questions)],
    [pageData],
  );
  ...
  const updateQuestion = (index, updatedQuestion) => {
    setQuestions((prevQuestions) => {
      ...
      if (updatedQuestion.id && updatedQuestion.id !== newQuestions[index].id) {
        const existingIndex = newQuestions.findIndex(question => question.id === updatedQuestion.id);
        if (existingIndex !== -1) {
  ...
  return (
    <div className="flex h-full flex-col relative">
      {isChanged && (
        <div className="flex justify-between px-4 absolute w-full py-2 bg-accent items-center" style={{top:"-65px"}}>
          <Button onClick={handleReset} color="accent">Reset</Button>
          <div className="text-xl font-Zilla-Slab">You have unsaved changes</div>
