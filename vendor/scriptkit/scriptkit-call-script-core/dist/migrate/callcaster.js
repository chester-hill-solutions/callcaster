import { z } from "zod";
import { callcasterFlowSchema, } from "../types.js";
...
function collectExtras(raw, known) {
    const extras = {};
 ...
 */
function toWireType(block) {
    return (block.callcasterType ?? DOC_TO_CALLCASTER_TYPE[block.type] ?? block.type);
}
...
function normalizeCallcasterBlock(id, raw) {
    ...
    const options = Array.isArray(parsed.options)
        ? parsed.options.map((opt) => {
            ...
            const value = String(record.value ?? record.id ?? record.content ?? record.label ?? "");
...
export function migrateFromCallcasterFlow(flow) {
    ...
    for (const [pageId, page] of Object.entries(parsed.pages)) {
        ...
        pages[id] = {
            ...
            wireExtras: collectExtras(page, KNOWN_PAGE_KEYS),
        };
    }
    ...
    const startPageId = parsed.startPageId && pages[parsed.startPageId]
        ? parsed.startPageId
