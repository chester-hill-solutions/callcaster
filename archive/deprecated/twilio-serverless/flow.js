...
const getCallWithRetry = async (supabase, callSid, retries = 0) => {
  ...
  if (error || !data) {
    log("warn", `Failed to retrieve call data`, {
      callSid,
      error,
...
const findNextStep = (currentBlock, userInput, script, pageId) => {
  if (currentBlock.options && currentBlock.options.length > 0) {
    const matchedOption = currentBlock.options.find((option) => {
      ...
      return (
        optionValue === input || (input.length > 0 && optionValue === "vx-any")
      );
    });
    if (matchedOption) {
...
const handleVMAudio = async (supabase, campaign, workspace) => {
  ...
  try {
    ...
    log("error", `Failed to create signed URL for voicemail`, {
      error: error.message,
    });
    return null;
  }
...
exports.handler = async function (context, event, callback) {
  ...
  try {
    ...
    log("info", "Retrieved call data", {
      ...
      scriptBlocks: Object.keys(
        callData.campaign.ivr_campaign[0].script.steps.blocks,
      ).length,
      campaignId: callData.campaign?.id,
    });

    let currentStep =
      callData.outreach_attempt?.current_step || "page_1:block_1";
    ...
    if (
      event.AnsweredBy?.includes("machine") &&
      !event.AnsweredBy.includes("other")
    ) {
      if (!callData.campaign.voicemail_file) {
        twiml.hangup();
      } else {
        const audio = await handleVMAudio(
          supabase,
          callData.campaign,
          callData.workspace,
        );
    ...
    if (currentStep === "hangup") {
      ...
      if (userInput) {
        const { step: newStep, result: newResult } = await processResult(
          supabase,
          script,
          currentStep,
          result,
