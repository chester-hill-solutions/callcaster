...
`Effort` is not an issue label or a GitHub Project field in this repository. It is an organization-level GitHub Issue field that every organization issue has; inspect that field with the GitHub API before selecting issues by effort.
