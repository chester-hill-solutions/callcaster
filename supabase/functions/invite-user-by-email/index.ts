...
async function inviteNewUser(email: string, workspaceId: string, role: string): Promise<any> {
  ...
  await sendInviteEmail(email, { hashed_token: data.properties.hashed_token });
  return data;
}
