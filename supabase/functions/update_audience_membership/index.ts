...
const evaluateCondition = (contact, condition)=>{
...
const handleAudienceRulesChange = async (supabase, operation, changedRule)=>{
  ...
  const { data: audience, error: audienceError } = await supabase.from("audience").select("is_conditional").eq("id", changedRule.audience_id).single();
  ...
  const { data: allRules, error: rulesError } = await supabase.from("audience_rule").select("*").eq("audience_id", changedRule.audience_id);
  ...
  if (operation === "DELETE" && allRules.length === 0) {
    ...
    await batchDelete(supabase, "contact_audience", {
      audience_id: changedRule.audience_id
    });
    return;
  }
  let hasMore = true;
  ...
  while(hasMore){
    ...
    const { data: contacts, error } = await supabase.from("contact").select("*").order('id').gt('id', lastId).limit(BATCH_SIZE);
    ...
    if (toAdd.length > 0) {
      ...
      await batchUpsert(supabase, "contact_audience", toAdd, [
        'contact_id',
        'audience_id'
      ]);
...
const handleContactChange = async (supabase, operation, contact)=>{
  ...
  const { data: audiences, error: audiencesError } = await supabase.from("audience").select(`
      id, 
      is_conditional,
      audience_rule (*)
  ...
  if (audiencesToAdd.length > 0) {
    await batchUpsert(supabase, "contact_audience", audiencesToAdd, [
      'contact_id',
      'audience_id'
    ]);
  }
...
const handleEvent = async (source, type, record, old_record)=>{
  ...
  try {
    if (source === "audience_rule") {
      await handleAudienceRulesChange(supabase, type, type === "DELETE" ? old_record : record);
    } else if (source === "contact") {
      await handleContactChange(supabase, type, type === "DELETE" ? old_record : record);
    }
  } catch (error) {
