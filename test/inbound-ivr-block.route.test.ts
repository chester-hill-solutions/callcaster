...
describe("inbound IVR block", () => {
  test("nests the prompt inside <Gather> so a key press interrupts it (#1841)", async () => {
    ...
    expect(xml).toContain(
      '<Gather action="https://base.example/api/inbound-ivr/1/page_1/b1/response" input="dtmf speech" speechTimeout="auto" speechModel="phone_call" timeout="5"><Play>https://signed</Play></Gather>',
    );
    expect(xml).toContain(
