...
describe("app/routes/api+/docs/openapi/route.tsx", () => {
  ...
  test("returns 405 when method not GET", async () => {
    ...
    const res = await asRouteResponse(mod.loader({ request: new Request("http://x", { method: "POST" }) } as any));
  ...
  test("returns spec and cache headers on GET", async () => {
    const mod = await import("../app/routes/api+/docs/openapi.route");
    const res = await asRouteResponse(mod.loader({ request: new Request("http://x", { method: "GET" }) } as any));
